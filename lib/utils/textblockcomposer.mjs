...
import Countries from "../countries/countries.mjs";
...
export default class TextBlockComposer {
            ...
            for ( let elem = 0; elem < lineLength; elem++ ) {
                ...
                switch ( element.type ) {
                    case 'value' :
                        ...
                        break;
                    case 'country' :
                        let countryCode = this.getValue( object, element.field );
                        //--- todo: MN-ERIKA FIX:
    ...
    getValue(object, path) {
        ...
        for (const attribute of attributes) {
            if (value &&  ( attribute in value || value.hasOwnProperty(attribute) || true ) ) {
